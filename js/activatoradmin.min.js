...
$("#background").show("fast","linear",function(){$("#container").append('<img class="img-thumbnail img-full" src="'+a+'" style="position: absolute; top: '+b+"px; left: "+d+'px" />')});$("#background").on("click",function(){$(this).hide();$("#container").find(".img-full").remove()})}});window.ActivatorPaginationView=Backbone.View.extend({events:{"click a.first":"gotoFirst","click a.prev":"gotoPrev","click a.next":"gotoNext","click a.last":"gotoLast","click a.page":"gotoPage"},template:_.template(f),
initialize:function(){b.on("reset",this.render,this)},render:function(){b.currentPage==b.firstPage&&b.origModels.length<=b.perPage||(this.$el.html(this.template(b.info())),$("#pagination-container").append(this.$el))},gotoFirst:function(a){a.preventDefault();b.goTo(1)},gotoPrev:function(a){a.preventDefault();b.previousPage()},gotoNext:function(a){a.preventDefault();b.nextPage()},gotoLast:function(a){a.preventDefault();b.goTo(b.information.lastPage)},gotoPage:function(a){a.preventDefault();a=$(a.target).text();
b.goTo(a)}});window.ActivatorAppView=Backbone.View.extend({el:$("#container"),initialize:function(){b.on("reset",this.addItems,this);b.fetch({reset:!0,success:function(a,c){b.pager();(new ActivatorPaginationView).render()}})},addItems:function(){$("#itemlist").empty();b.each(function(a){a=new ActivatorItemView({model:a});$("#itemlist").append(a.render())})}});new ActivatorAppView});
